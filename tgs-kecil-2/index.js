...
let petList = JSON.parse(savedPetList)
...
const PetCardComponent = (pet) => {
  ...
  const breed_1 = pet.breeds[0] || {name : 'Unknown Breed'};
  
  console.log(pet.breeds.length)
  ...
  return `<div class="card my-3 mx-2" style="width: 20%">
    ...
    <div class="card-body">
      <h5 class="card-title d-inline">${breed_1.name}</h5>
      <p class="card-text">
        ${breed_1.temperament || "No description"} 
      </p>
      <p>${breed_1.life_span || "Unknown life span"}</p>
      <span class="badge badge-pill badge-info">${breed_1.bred_for}</span>
      <span class="badge badge-pill badge-warning">Weight: ${breed_1.weight.imperial || "Unknown weight"}</span>
      <span class="badge badge-pill badge-danger">Height: ${breed_1.height.imperial|| "Unknown height"}</span>
    </div>
  </div>`;
};
...
const searchPetByKey = (key) => {
  ...
  {
    const breed1 = pet.breeds[0] || { name: "Unknown Breed" };
    const breedName = breed1.name.toLowerCase();
...
formElement.addEventListener("submit", (event) => {
  ...
  const filteredPet = searchPetByKey(key);
